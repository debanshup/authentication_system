...
const Signup = () => {
    ...
    const handleClosePasswordMismatchPopUp = () => setShowPasswordMismatchPopUp(false);
        ...
        <div className='container-fluid border d-flex flex-column justify-content-center align-items-center vh-100' style={{ background: 'white' }}>
            <form style={{ backdropFilter: 'blur(50px)' }} className='shadow-lg rounded p-4  col-xxl-3 col-xl-4 col-lg-5 col-md-6 col-sm-8'>
                ...
                <div className="mb-3">
                    ...
                    <input onChange={(e) => { setUser({ ...user, username: e.target.value }) }} type="text" className="form-control" required />
                </div>
                <div className="mb-3">
                    <label htmlFor="exampleInputEmail1" className="form-label">Email</label>
                    <input onChange={(e) => { setUser({ ...user, email: e.target.value }) }} type="email" className="form-control" required />
                ...
                <div className="mb-3">
                    ...
                    <input onChange={(e) => { setUser({ ...user, password: e.target.value }) }} type="password" className="form-control" minLength={8}
                        required />

                    <div id="emailHelp" className="form-text">Password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one number, and one special character.</div>

                </div>
                <div className="mb-3">
                    <label htmlFor="exampleInputEmail1" className="form-label">Confirm password</label>
                    <input onChange={(e) => { setUser({ ...user, confirmedPassword: e.target.value }) }} type="password" className="form-control" minLength={8}
                        required />
                </div>

                <div className='text-end'>
                    <button onClick={nextBtnClickHandler} type="button" className="btn btn-primary btn-lg">Next</button>
                </div>
                <hr />
                <div className='m-3'>
                    <div className='mb-3 text-center'>
                        <span>
                            Already have an account? <Link className='link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover' href={'/login'}>Log in</Link>
                        </span>
            ...
            {showEmailSentPopUp &&
                <EmailSentPopup show={showEmailSentPopUp} handleClose={handleCloseEmailSentPopUp} email={user.email} />

            }
            {showInputErrorPopUp &&
                <InputErrorPopup show={showInputErrorPopUp} handleClose={handleCloseInputErrorPopUp} />
            }
            {showPasswordMismatchPopUp &&
                <PasswordMismatchPopUp show={showPasswordMismatchPopUp} handleClose={handleClosePasswordMismatchPopUp} />
            }
            {showUserExistPopup &&
                <UserExistPopup show={showUserExistPopup} handleClose={handleCloseUserExistPopup} email={user.email} />
            }
            {showSignupErrorPopup &&
                <SignupErrorPopup show={showSignupErrorPopup} handleClose={handleCloseSignupErrorPopup} email={user.email} />
            }
            {showUnknownErrorPopup &&
                <UnknownErrorPopup show={showUnknownErrorPopup} handleClose={handleCloseUnknownErrorPopup} email={user.email} />
            }
        </div>
